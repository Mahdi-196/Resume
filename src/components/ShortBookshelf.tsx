// Short Bookshelf Component - Half height version of the regular bookshelf
export const ShortBookshelf = ({ position, rotation = [0, 0, 0], variant = 0 }: { 
  ...
  const generateShelfBooks = (shelfIndex: number) => {
    ...
    for (let i = 0; i < bookCount; i++) {
      ...
      books.push({
        height: 0.2 + seededRandom(seed) * 0.3, // 0.2 to 0.5
        thickness: 0.02 + seededRandom(seed + 1) * 0.03, // 0.02 to 0.05
        color: Math.floor(seededRandom(seed + 2) * bookColors.length),
        lean: 0 // Keep disabled
      });
    }
    return books;
  };
  ...
  return (
    <group position={position} rotation={rotation}>
      {/* Bookshelf frame - half height (2.5 units) */}
      ...
      {/* Shelves - 2 shelves for half height bookshelf */}
      {[0.8, 1.6].map((y, i) => (
        ...
        return (
          <group key={`books-${shelfIndex}`}>
            {shelfBooks.map((book, bookIndex) => {
              ...
              // Stop if we've filled the shelf
              if (currentX > 0.8) return null;
              
              // Clamp book height to stay within shelf compartment
              const maxHeight = 0.5;
      ...
      {variant % 5 === 2 && (
        <mesh position={[0.3, 1.25, -0.6]} rotation={[Math.PI/2, 0, 0]}>
